...
class Order(StatusModel):
    ...
    shop_uuid = models.ForeignKey(Shop, related_name='shop_order')
...
class ShopBucket(models.Model):
    ...
    product_uuid = models.ForeignKey(Product, blank=False, null=False, default=None)
    value = models.DecimalField(max_digits=8, decimal_places=2)


class OrderProducts(models.Model):
    order_product_uuid = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    order_uuid = models.ForeignKey(Order, related_name='order_orderproducts', on_delete=models.CASCADE)
    quantity = models.SmallIntegerField(default=0)
    product_uuid = models.ForeignKey(Product, blank=False, null=False, default=None, related_name='product_orderproducts')
    value = models.DecimalField(max_digits=8, decimal_places=2)
